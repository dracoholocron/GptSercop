...
-- Usamos procedimiento almacenado para verificar si las columnas existen
...
-- Agregar columnas a external_api_call_log
CALL add_column_if_not_exists('external_api_call_log', 'call_id', 'VARCHAR(36) AFTER id');
CALL add_column_if_not_exists('external_api_call_log', 'mapped_response_data', 'JSON AFTER response_body');
CALL add_column_if_not_exists('external_api_call_log', 'listeners_executed', 'JSON AFTER mapped_response_data');
CALL add_column_if_not_exists('external_api_call_log', 'status', "ENUM('PENDING', 'SUCCESS', 'FAILED', 'TIMEOUT', 'RETRY') DEFAULT 'PENDING' AFTER success");
CALL add_column_if_not_exists('external_api_call_log', 'aggregate_id', 'VARCHAR(100) AFTER listeners_executed');
CALL add_column_if_not_exists('external_api_call_log', 'response_timestamp', 'TIMESTAMP NULL AFTER response_body');

-- Agregar índices
CALL add_index_if_not_exists('external_api_call_log', 'idx_api_call_log_call_id', 'call_id');
CALL add_index_if_not_exists('external_api_call_log', 'idx_api_call_log_aggregate', 'aggregate_id');
...
SELECT r.id, 'CAN_MANAGE_API_MAPPINGS'
FROM role_read_model r
WHERE r.name = 'ROLE_ADMIN'
AND NOT EXISTS (
    SELECT 1 FROM role_permission_read_model
