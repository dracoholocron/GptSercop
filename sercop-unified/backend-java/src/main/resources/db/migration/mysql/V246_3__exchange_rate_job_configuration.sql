-- ============================================================================
-- V246_3__exchange_rate_job_configuration.sql
...
-- Create with code CORE_EXCHANGE_RATES (for manually created jobs) and FRANKFURTER_EXCHANGE_RATES
INSERT INTO external_api_config_read_model (
    ...
    'CORE_EXCHANGE_RATES',
    'Exchange Rates API (Frankfurter)',
...
-- Also create with alternate code for compatibility
INSERT INTO external_api_config_read_model (
    code, name, description, base_url, path, http_method, content_type,
    timeout_ms, retry_count, retry_initial_delay_ms, retry_backoff_multiplier,
    active, mock_enabled, environment, created_at
...
-- Get the API config IDs
SET @api_config_id = (SELECT id FROM external_api_config_read_model WHERE code = 'FRANKFURTER_EXCHANGE_RATES');
SET @core_api_config_id = (SELECT id FROM external_api_config_read_model WHERE code = 'CORE_EXCHANGE_RATES');

-- 2. Request Mappings for CORE_EXCHANGE_RATES (primary config)
...
-- Response Mappings for CORE_EXCHANGE_RATES
...
-- Response Listener for CORE_EXCHANGE_RATES
...
-- 3. Request Mappings for FRANKFURTER_EXCHANGE_RATES (alternate config)
-- Base currency (from)
INSERT INTO external_api_request_mapping (
    api_config_id, source_type, variable_code, constant_value, calculated_expression,
    api_parameter_name, parameter_location, is_required, display_order, is_active, description
