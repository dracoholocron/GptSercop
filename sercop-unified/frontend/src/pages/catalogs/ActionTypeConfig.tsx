import { Box, Input, VStack, Text, Flex, Button, HStack, DialogRoot, DialogContent, DialogHeader, DialogBody, DialogFooter, DialogTitle, DialogBackdrop, DialogCloseTrigger, Grid, Badge, Spinner, DialogActionTrigger, Heading, Textarea } from '@chakra-ui/react';
...
type FormData = Omit<ActionTypeConfig, 'id' | 'createdAt'>;
...
export const ActionTypeConfigPage = () => {
  const { t } = useTranslation();
  const { getColors } = useTheme();
  const { bgColor, borderColor, cardBg, textColor, textColorSecondary, primaryColor } = getColors();

  const [configs, setConfigs] = useState<ActionTypeConfig[]>([]);
  const [loading, setLoading] = useState(true);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingConfig, setEditingConfig] = useState<ActionTypeConfig | null>(null);
  ...
  const handleEdit = (config: ActionTypeConfig) => {
  ...
  const handleToggleActive = async (config: ActionTypeConfig) => {
    try {
  ...
  const columns: DataTableColumn<ActionTypeConfig>[] = [
    { key: 'actionType', label: t('actionTypeConfig.actionType'), render: (row) => <Badge colorPalette="purple">{row.actionType}</Badge> },
    { key: 'displayName', label: t('actionTypeConfig.displayName') },
    {
      key: 'language', label: t('common.language'),
      filterType: 'select', filterOptions: [{ value: 'es', label: 'Español' }, { value: 'en', label: 'English' }],
      render: (row) => <Badge colorPalette={row.language === 'es' ? 'blue' : 'green'}>{row.language.toUpperCase()}</Badge>,
    },
    { key: 'icon', label: t('actionTypeConfig.icon'), hideOnMobile: true, render: (row) => <Text>{row.icon || '-'}</Text> },
    { key: 'color', label: t('actionTypeConfig.color'), hideOnMobile: true, render: (row) => row.color ? <Badge colorPalette={row.color}>{row.color}</Badge> : <Text>-</Text> },
    { key: 'displayOrder', label: t('common.order'), hideOnMobile: true },
    {
  ...
  const actions: DataTableAction<ActionTypeConfig>[] = [
  ...
  return (
    <Box p={6}>
      <VStack align="stretch" gap={6}>
        <Heading size="lg" color={textColor}>{t('actionTypeConfig.title')}</Heading>

        <DataTable<ActionTypeConfig>
          data={configs}
          columns={columns}
      ...
      <DialogRoot open={isModalOpen} onOpenChange={(e) => setIsModalOpen(e.open)}>
        ...
        <DialogContent maxW="700px">
          <DialogHeader>
            <DialogTitle>
              {editingConfig ? t('actionTypeConfig.edit') : t('actionTypeConfig.create')}
            </DialogTitle>
            <DialogCloseTrigger />
          </DialogHeader>
          <DialogBody>
            <VStack gap={4} align="stretch">
              <Grid templateColumns="repeat(2, 1fr)" gap={4}>
                <Box>
                  ...
                  <Input
                    ...
                    onChange={(e) => setFormData({ ...formData, actionType: e.target.value.toUpperCase() })}
                    placeholder="SWIFT_MESSAGE, API_CALL, EMAIL..."
                  />
                </Box>
                <Box>
              ...
              <Box>
                ...
                <Input
                  ...
                  placeholder={t('actionTypeConfig.displayNamePlaceholder')}
                />
              </Box>
              ...
              <Grid templateColumns="repeat(2, 1fr)" gap={4}>
                <Box>
                  ...
                  <Input
                    value={formData.retryMessage}
                    onChange={(e) => setFormData({ ...formData, retryMessage: e.target.value })}
                  />
                </Box>
                <Box>
                  ...
                  <Input
                    ...
                    onChange={(e) => setFormData({ ...formData, skipMessage: e.target.value })}
                  />
                </Box>
              </Grid>
...
export default ActionTypeConfigPage;
